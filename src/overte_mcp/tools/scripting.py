    ...
    try:
        async with httpx.AsyncClient(timeout=6.0) as client:
            ...
            if r.status_code == 200:
                logger.info(f"Successfully injected script into entity {entity_id} via local REST API bridge")
                return r.json()
        ...
        logger.debug(f"HTTP request to local inject REST server failed: {e}. Falling back to simulation.")

    logger.info(
        f"overte-mcp: script injection falling back to simulated for entity {entity_id}"
    )
    return {
        "status": "success",
        "source": "simulated",
